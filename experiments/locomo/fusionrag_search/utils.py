...
def smart_query_selection(attention_scores, doc_len, target_ratio, system_len, device='cpu', smarter=False,
                          eigenvalue=None, tokenizer=None, input_tokens=None, similarity=0.0, keyword="", must_choose_token_indices_without_system=[]):
    """
    ...
    target_count = int((doc_len - len(must_choose_token_indices_without_system)) * target_ratio)
        ...
        for p in comp:
            if "no_smart_connect" in keyword:
                ...
                for offset in range(-1, 2):
                    new_p = p + offset
                    if 0 <= new_p < doc_len:
                        if new_p not in must_choose_token_indices_without_system:
    ...
    for cs in component_scores_:
        if cs not in component_scores:
            ...
            extended_comp = set()
            for p in comp:
                if p not in must_choose_token_indices_without_system:
                    extended_comp.add(p)
    ...
    if not smarter:
        if len(selected) < target_count:
            ...
            for pos in sorted_indices:
                if pos not in selected and pos not in must_choose_token_indices_without_system:
...
def find_all_substr_needs_recompute(draft_model, draft_model_device, tokenizer, system_prompt: str,
    ...
    must_choose_token_indices_without_system = copy.deepcopy(must_choose_token_indices)
    must_choose_token_indices_without_system = [x-len(system_prompt_tokens) for x in must_choose_token_indices_without_system]
    if reverse_attn:
        selected_indices = smart_query_selection(
            ...
            keyword=keyword,
            must_choose_token_indices_without_system=must_choose_token_indices_without_system
        )
    else:
        selected_indices = smart_query_selection(
            attention_scores=multi_layer_attn,
